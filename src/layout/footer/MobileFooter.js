  ...
  return (
    <>
      ...
      <div className="flex flex-col h-full justify-between align-middle bg-white rounded cursor-pointer overflow-y-scroll flex-grow scrollbar-hide w-full">
       {categories==undefined?(<CategoryDrawer className="w-6 h-6 drop-shadow-xl" />):(<CategoryDrawer className="w-6 h-6 drop-shadow-xl" categories={categories} />)} 
        
      </div>
      <footer className="lg:hidden fixed z-30 bottom-0 bg-[#ec3852] flex items-center justify-between w-full h-16 px-3 sm:px-10">
        <button
          aria-label="Bar"
        ...
        <FiAlignLeft className="w-6 h-6 drop-shadow-xl" />
         ...
         <span className="text-xl text-white">Categories</span> 
        ...
        >
          <span className="absolute z-10 top-0 right-0 inline-flex items-center justify-center p-1 h-5 w-6 text-base font-bold leading-none text-white-100 transform translate-x-1/2 bg-white-500 rounded-full">
            {totalItems}
          </span>
        ...
        >
          {userInfo?.image ? (
            ...
            <Link href="/user/dashboard">
              <a className="leading-none font-xs font-serif block">
                <ImUser/>
              </a>
            </Link>
          ) : (
